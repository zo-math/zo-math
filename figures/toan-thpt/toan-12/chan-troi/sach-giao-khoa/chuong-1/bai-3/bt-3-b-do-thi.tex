...
\begin{tikzpicture}
  ...
  \node[originlabel] at (axis cs:0,0) {\small$O$};
  \node at (axis cs:0,1) [anchor=north west] {\footnotesize$1$};
  \node at (axis cs:-1,0) [below] {\footnotesize$-1$};
  
  \addplot[domain=-4:4, color=den-2, line width=1.5pt, opacity=.8] {(x^2+x-1)/x};
  
  \addplot[dashed, smooth, domain=-4:4, color=den-2, line width=1.5pt, opacity=.8] {x+1};

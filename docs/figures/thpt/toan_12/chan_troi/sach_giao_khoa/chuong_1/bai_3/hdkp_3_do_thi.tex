...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \node at (axis cs:1,1) [below right] {$N$};
    \node at (axis cs:1,2) [above] {$M$};

    \addplot[domain=-5:5, samples=200, line width=1.5pt, color=den-2, opacity=.8] 
        {(x^2+1)/x};
    \addplot[domain=-5:5, samples=200, line width=1.5pt, color=do-2, opacity=.8] 
        {x};

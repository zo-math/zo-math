...
\begin{tikzpicture}[zo graph field]
  \begin{axis}[
    ...
    ymin=-1.8,
    ymax=1.8,
    xtick={-1.5707963268,-1,1,1.5707963268},
    ytick={-1.5707963268,-1,1,1.5707963268},
    xticklabel=\empty,
    yticklabel=\empty,
    xlabel={$x$},
    ylabel={$y$},
    xlabel style={at={(axis description cs:1.025,0.515)},anchor=west},
    ylabel style={at={(axis description cs:0.515,1.025)},anchor=south}
  ]
    % Đường y=x là trục đối xứng của hai đồ thị hàm ngược.
    \addplot[zo reference,domain=-1.62:1.62,samples=2] {x};

    % Nhánh sin chính: miền lấy mẫu đúng bằng [-pi/2, pi/2].
    \addplot[zo graph secondary,domain=-1.5707963268:1.5707963268,samples=181]
      {sin(deg(x))};

    % Hàm arcsin: asin của PGF trả về độ, nên đổi sang radian để khớp trục y.
    \addplot[zo graph main,domain=-1:1,samples=201]
      {asin(x)*pi/180};

    % Hai đầu mút thuộc đồ thị arcsin.
    \addplot[zo point closed] coordinates {
      (-1,-1.5707963268)
      (1,1.5707963268)
    };

    % Nhãn giá trị được đặt thủ công tại đúng tọa độ toán học.
    \node[zo tick label,anchor=north,yshift=-2pt]
      at (axis cs:-pi/2,0) {$-\frac{\pi}{2}$};
    \node[zo tick label,anchor=north,yshift=-2pt]
      at (axis cs:-1,0) {$-1$};
    \node[zo tick label,anchor=north,yshift=-2pt]
      at (axis cs:1,0) {$1$};
    ...
    \node[zo tick label,anchor=east,xshift=-2pt]
      at (axis cs:0,-pi/2) {$-\frac{\pi}{2}$};
    \node[zo tick label,anchor=east,xshift=-2pt]
      at (axis cs:0,-1) {$-1$};
    \node[zo tick label,anchor=east,xshift=-2pt]
      at (axis cs:0,1) {$1$};
    ...
    \node[zo origin label,anchor=north east,xshift=-1pt,yshift=-1pt]
      at (axis cs:0,0) {$O$};

    \node[zo reference label,anchor=south west]
      at (axis cs:1.27,1.27) {$y=x$};
    \node[zo graph label,text=zoGraphMain,anchor=west]
      at (axis cs:0.64,1.12) {$y=\arcsin x$};
    \node[zo graph label,text=zoGraphSecondary,anchor=west]
      at (axis cs:1.02,0.68) {$y=\sin x$};
  \end{axis}
\end{tikzpicture}
\end{document}

...
\begin{tikzpicture}[zo graph field]
  \begin{axis}[
    zo axis,
    xmin=-0.35,xmax=4.6,ymin=-0.35,ymax=2.8,
    xtick={1,4},xticklabel=\empty,
    ytick={1,2},yticklabel=\empty
  ]
    \addplot[zo graph secondary,domain=0:4.5,samples=2] {(x+1)/2}
      node[pos=0.82,above,sloped,zo graph label,text=zoGraphSecondary] {$y=\dfrac{x+1}{2}$};
    \addplot[zo graph main,domain=0:4.5,samples=160] {sqrt(x)};
    \addplot[zo reference] coordinates {(1,0) (1,1)};
    \addplot[zo reference] coordinates {(0,1) (1,1)};
    \addplot[zo point emphasized] coordinates {(1,1)};
    ...
    \node[zo axis label,anchor=west,xshift=2pt] at (axis description cs:1,0.1111) {$x$};
    \node[zo axis label,anchor=south,yshift=2pt] at (axis description cs:0.0707,1) {$y$};
    \node[zo graph label,anchor=south east,text=zoGraphMain] at (axis cs:4.35,2.12) {$y=\sqrt{x}$};
  \end{axis}
\end{tikzpicture}
\end{document}

...
\begin{axis}[
    ...
    % Draw the main graph
    \addplot[zo graph main] {x^3};

    % Points and labels
    \node[zo origin label, anchor=north west, xshift=1pt, yshift=-1pt] at (axis cs:0,0) {$O$};
    ...
    % Auxiliary lines
    \addplot[zo auxiliary line] coordinates {(1,0) (1,1) (0,1)};
    \addplot[zo auxiliary line] coordinates {(-1,0) (-1,-1) (0,-1)};
    
    % Marked points
    \addplot[zo point closed] coordinates {(0,0)};
    \addplot[zo point closed] coordinates {(1,1)};
    \addplot[zo point closed] coordinates {(-1,-1)};
    
    % Graph label
    \node[zo graph label, anchor=east, xshift=-3pt, yshift=2pt] at (axis cs:1.18, 2.65) {$y=x^3$};
